...
namespace fgui {
    export class GComponent extends GObject {
        ...
        public _childrenHash: {[key: string]: GObject};
        ...
        protected genPartner() {
            this._partner = this._node.addComponent(GActivatePartner);
        }
        ...
        public addChildren(children: Array<GObject>) {
            if (!children) return;
            ...
            for (i = 0; i < len; ++ i) {
                ...
                //     child.node.setSiblingIndex(this._children.length);
                // else if (this._childrenRenderOrder == ChildrenRenderOrder.Descent)
                //     child.node.setSiblingIndex(cnt - this._children.length);
                // else
        ...
        public applyController(c: Controller, callback?: Function): void {
        ...
        public constructFromResource2(objectPool: Array<GObject>, poolIndex: number): void {
            ...
            */
            ...
            // let t = t2 - t1;
            // if (!GComponent.profile[this.packageItem.name]) {
            //     GComponent.profile[this.packageItem.name] = 0;
            // }
            // GComponent.profile[this.packageItem.name] += t;
        }
