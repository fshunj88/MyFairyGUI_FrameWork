namespace fgui {
    export class Transition {
        ...
        public setTransitionItemEndXYByLabel(label: string, x: number, y: number): void {
            var cnt: number = this._items.length;
            var targetItem: TransitionItem = null;
            for (var i: number = 0; i < cnt; i++) {
                var item: TransitionItem = this._items[i];
                if (item.label == label) {
                    targetItem = item;
        ...
        public setTransitionItemBeginXYByLabel(label: string, x: number, y: number): void {
            var cnt: number = this._items.length;
            var targetItem: TransitionItem = null;
            for (var i: number = 0; i < cnt; i++) {
                var item: TransitionItem = this._items[i];
                ...
                case TransitionActionType.Sound:
                    if (this._playing && item.time >= this._startTime) {
                        if (value.audioClip == null) {
                            GRoot.inst.playSoundAsync(value.sound, value.volume);
                        }
                    }
                    break;
