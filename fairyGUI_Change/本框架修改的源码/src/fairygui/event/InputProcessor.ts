...
namespace fgui {
    ...
    export class InputProcessor extends cc.Component {
        ...
        private static onDispatchEvent(touch: cc.Touch, target: fgui.GObject, opType: string) {
            InputProcessor._eventListener.forEach((listener)=>{
        ...
        public simulateTouchBegin(touch: cc.Touch, target: fgui.GObject) {
        ...
        public simulateTouchMove(touch: cc.Touch, target: fgui.GObject) {
            ...
            if (ti.began) {
                ...
                let offx = ti.downPos.x - ti.pos.x;
                let offy = ti.downPos.y - ti.pos.y;
                if (!ti.hasMove && (Math.abs(offx) >= UIConfig.touchDragSensitivity || Math.abs(offy) >= UIConfig.touchDragSensitivity)) {
                    ti.hasMove = true;
                }
                let done = false;
        ...
        public simulateTouchEnd(touch: cc.Touch, target: fgui.GObject) {
            let ti = this.updateInfo(touch.getID(), touch.getLocation(), touch);
            ti.target = target;
            this.setEnd(ti);
            ...
            let tiCnt = this._touches.length;
            let i = 0;
            for (; i < tiCnt; ++ i) {
        ...
        public simulateTouchCancel(touch: cc.Touch, target: fgui.GObject) {
            let ti = this.updateInfo(touch.getID(), touch.getLocation(), touch);
            ti.target = target;
            let evt2 = this.getEvent(ti, ti.target, Event.TOUCH_END, false);
        ...
        public simulateTouchBegin1(touch: cc.Touch, target: fgui.GObject) {
            let ti: TouchInfo = this.updateInfo(touch.getID(), touch.getLocation(), touch);
            ti.target = target;
            this._touchListener.setSwallowTouches(ti.target != this._owner);
            this.setBegin(ti);
        ...
        private touchEndHandler(touch: cc.Touch, evt: cc.Event): void {
            ...
            ti.target = this.clickTest(ti);
            if (ti.target && !ti.hasMove) {
                InputProcessor.onDispatchEvent(touch, ti.target, Event.CLICK);
                evt2 = this.getEvent(ti, ti.target, Event.CLICK, true);
                ti.target.node.dispatchEvent(evt2);
                Event._return(evt2);
            }
        ...
        public updateInfo(touchId: number, pos: cc.Vec2, touch?: cc.Touch): TouchInfo {
