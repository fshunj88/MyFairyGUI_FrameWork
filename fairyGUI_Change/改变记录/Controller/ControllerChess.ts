...
namespace fgui {
    ...
    export class Controller extends cc.EventTarget {
        ...
        private _runningCallback: Function;
        ...
        public setAndWaitSelectedIndex(value: number, callback: Function): void {
            // if (this._selectedIndex != value) {
                if (value > this._pageIds.length - 1)
                    throw "index out of bounds: " + value;

                // console.log("[Controller] setAndWaitSelectedIndex ", value);
                this.changing = true;
                this._previousIndex = this._selectedIndex;
                this._selectedIndex = value;
        ...
        public runActions(callback?: Function): void {
            if (this._actions) {
                var cnt: number = this._actions.length;
                this._runningActions = cnt;
                this._runningCallback = callback;
                // console.log("[Controller] runActions ", this._runningActions, this._runningCallback);
                for (var i: number = 0; i < cnt; i++)
                    this._actions[i].run(this, this.previousPageId, this.selectedPageId);
            }
        }

        public onActionRunDone() {
            -- this._runningActions;
            // console.log("[Controller] onActionRunDone ", this._runningActions, this._runningCallback);
