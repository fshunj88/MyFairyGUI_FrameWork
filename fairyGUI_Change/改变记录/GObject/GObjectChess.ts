...
namespace fgui {
    ...
    export let NEW_ENGINE_VERSION: string = "2.2.2";
    ...
    export class GObject {
        ...
        protected _originVisible: boolean = true;
        ...
        public _delayAddFlag: boolean = false;
        private _hashIdx: number;

        public static _defaultGroupIndex: number = -1;
        private static _gHashIdx = 0;

        private static _touchEventListener = [];
        public static addTouchEventListener(cbFunc: (event: Event, posx: number, posy: number)=>{}, thisObj: any) {
            GObject._touchEventListener.push({callback: cbFunc, obj: thisObj})
        }
        public static _lastEvent = null;
        private static onDispatchEvent(event: cc.Event.EventTouch) {
            console.log("fgui.touch eventType", event);
            GObject._touchEventListener.forEach((listener)=>{
                let func = listener.callback;
                let obj = listener.thisObj;
                // func();
        ...
        public static doDispatchEvent(event: cc.Event) {
        ...
        protected genPartner() {
            this._partner = this._node.addComponent(GObjectPartner);
        }

        public once<T extends Function>(type: string, callback: T, target?: any, useCapture?: boolean) {
            this.node.once(type, callback, target, useCapture);
        }

        public dispatchEventWith(type: string, bubble?: boolean, data?: any) {
        ...
        public saveAndSetVisible(value: boolean) {
        ...
        public localToGlobal(ax?: number, ay?: number, resultPoint?: cc.Vec2): cc.Vec2 {
            ...
            //pt.y = GRoot.inst.height * GRoot.contentScaleFactor - pt.y;
            let v3 = this._node.convertToWorldSpaceAR(pt);
            pt.x = v3.x;
            pt.y = GRoot.inst.height * GRoot.contentScaleFactor - v3.y;
            return pt;
        }
        ...
        public globalToLocal(ax?: number, ay?: number, resultPoint?: cc.Vec2): cc.Vec2 {
            ...
            let pt = resultPoint || new cc.Vec2();
            pt.x = ax;
            pt.y = GRoot.inst.height * GRoot.contentScaleFactor - ay;
        ...
        public localToRoot(ax?: number, ay?: number, resultPoint?: cc.Vec2): cc.Vec2 {
            // if (this._pivotAsAnchor) {
            //     ax -= this.width * this.pivotX;
            //     ay -= this.height * this.pivotY;
            // }
            // ax += (GRoot.inst.width - GRoot.inst.designWidth) / 2;
            // ay -= (GRoot.inst.height - GRoot.inst.designHeight) / 2;
            let pt = this.localToGlobal(ax, ay, resultPoint);
            pt.x /= GRoot.contentScaleFactor;
            pt.y /= GRoot.contentScaleFactor;
            pt.x -= (GRoot.inst.x / GRoot.contentScaleFactor);
            pt.y -= GRoot.inst.y;
            return pt;
        }

        public rootToLocal(ax?: number, ay?: number, resultPoint?: cc.Vec2): cc.Vec2 {
            ax += (GRoot.inst.x / GRoot.contentScaleFactor);
            ay += GRoot.inst.y;
            ax *= GRoot.contentScaleFactor;
            ay *= GRoot.contentScaleFactor;
            let pt = this.globalToLocal(ax, ay, resultPoint);
            // pt.x -= (GRoot.inst.width - GRoot.inst.designWidth) / 2;
            // pt.y += (GRoot.inst.height - GRoot.inst.designHeight) / 2;
            // if (this._pivotAsAnchor) {
            //     pt.x += this.width * this.pivotX;
            //     pt.y += this.height * this.pivotY;
            // }
            return pt;
        }
    ...
    export class GActivatePartner extends GObjectPartner {
        protected onEnable() {
            super.onEnable();
