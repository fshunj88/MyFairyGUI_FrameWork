...
namespace fgui {
    ...
    export class UIPackage {
        ...
        private static _curLanItem: UIPackage;
        ...
        // TODO
        public static loadPackage(url: string, loadRes: boolean, completeCallback: ((error: any) => void) | null): void {
        ...
        public static getPackage(packageIdOrName: string): UIPackage {
            var pkg: UIPackage = UIPackage._instByName[packageIdOrName];
            if (!pkg) {
        ...
        public get assetUrls(): string[] {
            return this._assetUrls;
        }

        public set assetUrls(urls: string[]) {
        ...
        private loadPackage(buffer: ByteBuffer, url: string): void {
            ...
            for (i = 0; i < cnt; i++) {
                ...
                if (type == PackageItemType.Image) {
                    pi = new PackageImageItem();
                } else if (type == PackageItemType.MovieClip) {
                    pi = new PackageMovieClipItem();
                } else if (type == PackageItemType.Font) {
                    pi = new PackageFontItem();
                } else if (type == PackageItemType.Component) {
                    pi = new PackageComponentItem();
                } else if (type = PackageItemType.Atlas) {
                    pi = new PackageAtlasItem();
                } else if (type == PackageItemType.Sound) {
                    pi = new PackageSoundItem();
                } else if (type == PackageItemType.Misc) {
                    pi = new PackageMiscItem();
                } else {
                    pi = new PackageItem();
                }
        ...
        public dispose(): void {
            // console.log("[UIPackage] disposing ", this.name)
            ...
            for (var i: number = 0; i < cnt; i++) {
                ...
                if (pi.asset) {
                    //@ts-ignore
                    if (pi.asset._uuid) {
                        let deps = cc.loader.getDependsRecursively(pi.asset);
                        if (deps) {
                            // console.log("[UIPackage] release by deps", pi.name, deps);
                            cc.loader.release(deps);
                        }
                    } else {
                        // console.log("[UIPackage] manual destroy asset", pi.name, pi.asset);
                        let nativeUrl = pi.asset.nativeUrl;
                        if (nativeUrl) {
                            cc.loader.release(nativeUrl);  // uncache loading item of native asset
                        }
                        pi.asset.destroy();
                    }
                }
            }
            // console.log("[UIPackage] asset urls = ", this._assetUrls);
            this._assetUrls.forEach(url => {
                let asset = cc.loader.getRes(url);
                // console.log("[UIPackage] check url", url, asset);
                if (asset) {
                    let deps = cc.loader.getDependsRecursively(asset);
                    // console.log("[UIPackage] release unused asset by deps", deps);
                    cc.loader.release(deps);
                }
            });
            this._assetUrls = [];
            // console.log("[UIPackage] disposed ", this.name);
        }
        ...
        public getAtlasSprite(idx: string) {
